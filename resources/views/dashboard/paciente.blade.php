...
<div class="space-y-6">
    ...
    <div
        class="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl shadow-lg p-6 text-white"
    >
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                ...
                <button
                    class="mt-4 bg-white text-cyan-600 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-gray-100 transition"
                >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-500 text-sm">
                            <i
                                class="fas fa-calendar-check text-green-500 mr-1"
                            ></i>
                            Próxima Cita
                        </p>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <p class="text-gray-500 text-xs">
                                ...
                                {{ date('h:i A', strtotime($proxima_cita->hora)) }}
                            </p>
                            @if ($proxima_cita->dentista)
                                <p class="text-xs text-cyan-600">
                                    <i class="fas fa-user-md mr-1"></i>
                                    {{ $proxima_cita->dentista->nombre }}
                                </p>
                            @endif
                        </div>
                    </div>
                    <div
                        class="w-12 h-12 bg-gradient-to-br from-orange-400 to-orange-600 rounded-lg flex items-center justify-center shadow-md"
                    >
        ...
        @endphp
                ...
                <div class="mt-3 flex gap-2">
                    ...
                    <a
                        href="{{ route('citas.reprogramar', $proxima_cita->id) }}"
            ...
            @else
                <div class="text-center py-6">
                    ...
                    >
                        <i
                            class="fas fa-calendar-times text-2xl text-gray-400"
                        ></i>
                    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-semibold text-slate-800">
                <i class="fas fa-calendar-week text-cyan-600 mr-2"></i>Mis
                Próximas Citas
            </h3>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-16 text-center">
                            <p class="text-2xl font-bold text-cyan-600">
                                {{ \Carbon\Carbon::parse($proxima_cita->fecha)->format('d') }}
                            </p>

                            <p class="text-xs text-gray-500">
                                {{ \Carbon\Carbon::parse($proxima_cita->fecha)->translatedFormat('M') }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">
                                ...
                                {{ \Carbon\Carbon::parse($proxima_cita->hora)->format('h:i A') }}

                                -

                                <i class="fas fa-tooth mr-1 ml-2"></i>

                                Dr. {{ $proxima_cita->dentista->user->nombre ?? 'N/A' }} {{ $proxima_cita->dentista->user->apellido_paterno ?? '' }}
                            </p>
                        </div>
                    </div>
        ...
        @else
            <div class="text-center py-8">
                <i
                    class="fas fa-calendar-times text-5xl text-gray-300 mb-3"
                ></i>
                <p class="text-gray-500">No tienes citas programadas</p>
                <button class="mt-3 text-cyan-600 hover:underline">
                    Agendar una cita
                </button>
            </div>
        @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <h3 class="font-semibold text-slate-800 mb-4">
                <i class="fas fa-history text-cyan-600 mr-2"></i>Últimas
                atenciones
            </h3>
            <div class="space-y-3">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <h3 class="font-semibold text-slate-800 mb-4">
                <i class="fas fa-credit-card text-cyan-600 mr-2"></i>Historial
                de Pagos
            </h3>
            ...
            <div class="space-y-3">
                ...
                ->translatedFormat('M') }}
                                ...
                                - ${{ number_format($pago->total_mes,2) }}
                            </p>
        ...
        <div
            class="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-6 border border-cyan-100"
        >
            <div class="flex items-start gap-3">
                <div class="text-3xl">🦷</div>
                <div>
                    <h4 class="font-semibold text-slate-800">
                        Consejo de salud dental
                    </h4>
